...
class Document extends DocumentManagerAppModel {
	...
	public function beforeValidate($cascade = true) {
		$this->validate = array(
			'url' => array(
	...
	function saveDocument($data, $pathFolderNames = array(), $userId, $headers = array()) {
		if (substr($data['file']['name'], 0, 1) == '.') {
		...
		} else if (empty($data['file']['error']) &&
				!empty($data['file']['tmp_name']) &&
				$data['file']['tmp_name'] != 'none' &&
			...
			return $this->saveFile(
							...
							, $data['file']['name']
							, $userId
							, empty($data['comments']) ? null : $data['comments']);
		} else {
			if (!empty($headers['Content-Length']) &&
					(int) $headers['Content-Length'] > ($max = $this->phpMaxUploadSize()) * 1024 * 1024) {
	 ...
	 */
	static function getRelativePath($pathFolderNames = array(), $fileName = '') {
