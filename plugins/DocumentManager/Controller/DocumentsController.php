...
class DocumentsController extends DocumentManagerAppController {
	...
	public function create_subfolder() {
		if (substr($this->request->data('folderName'), 0, 1) == '.') {
			...
			if ($error = $this->Document->createSubFolder($this->passedArgs, $this->request->data('folderName'))) {
				$this->Session->setFlash(__d("document_manager", $error), 'default', array('class' => 'alert alert-error'));
			}
		}
	...
	public function upload_file() {
		$message = $this->Document->saveDocument(
				$this->request->data
				, $this->passedArgs
				, $this->getUserId()
				, getallheaders());
		if (!empty($message)) {
			$this->Session->setFlash(__d("document_manager", $message), 'default', array('class' => 'alert alert-error'));
		}
		$this->redirect(array_merge($this->passedArgs, array('action' => 'index')));
	}
